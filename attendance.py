...
class Attendance:
    def __init__(self,root):
      ...
      #======VARIABLES=======
        ...
        title_lbl.place(x=250,y=0,width=1030,height=45)

        main_frame=Frame(bg_img,bd=2,bg="white")
        main_frame.place(x=20,y=95,width=1480,height=600)
        
            #left label frame
        Left_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student Attendance Details",font=("times new roman",35,"bold"))
        Left_frame.place(x=10,y=10,width=730,height=580)

        img_left=Image.open(r"D:\prayas2k25\prayas2k25\college images\facedetector.jpg")
        img_left=img_left.resize((720,130))#,Image.ANTIALIAS)
        self.photoimg_left=ImageTk.PhotoImage(img_left)

        f_lbl=Label(Left_frame,image=self.photoimg_left)
        f_lbl.place(x=750,y=0,width=720,height=130)
        ...
        #right  label frame
        Right_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Attendance Details",font=("times new roman",35,"bold"))
    ...
    def update_data(self):
        try:
            ...
            # Also update in the mydata list if needed
            index = self.AttendanceReportTable.index(selected)
            mydata[index] = updated_values

            messagebox.showinfo("Success", "Record updated successfully", parent=self.root)
        except Exception as e:
            messagebox.showerror("Error", f"Update failed: {str(e)}", parent=self.root)
